...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const data = await register({ phone: form.phone, firstName: form.firstName, lastName: form.lastName, pin: form.pin });
      setAuth({ token: data.token, user: data.user });
      toast.success("You're all set. Your first trade is one message away.");
      navigate('/dashboard');
    } catch (err) {
        ...
        {step === 1 ? (
          <form onSubmit={nextStep} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <Input
              ...
              placeholder="+2348012345678" error={errors.phone}
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            <Input
              label="Set PIN" type="password" inputMode="numeric" maxLength={6}
              value={form.pin} onChange={e => set('pin', e.target.value.replace(/\D/g, ''))}
              placeholder="••••" error={errors.pin}
            ...
            <Input
              ...
              placeholder="••••" error={errors.confirmPin}
            />
