          ...
          <section style={{ marginBottom: '2rem' }}>
            ...
            {pools.length === 0 ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1rem' }}>
                {pools.map(pool => (
                  <div key={pool.id}
                    onClick={() => pool.pool_type === 'fiat' && navigate(`/pools/${pool.id}`)}
                    style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '1.25rem', cursor: pool.pool_type === 'fiat' ? 'pointer' : 'default', transition: 'var(--trans-fast)' }}
                    onMouseEnter={e => { if (pool.pool_type === 'fiat') { e.currentTarget.style.borderColor = 'var(--teal)'; e.currentTarget.style.boxShadow = 'var(--shadow-sm)'; } }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.boxShadow = 'none'; }}
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                      <div>
                        <div style={{ fontWeight: 600, fontSize: '0.95rem', marginBottom: '0.2rem' }}>{pool.name}</div>
                        <span style={{ fontSize: '0.72rem', padding: '0.15rem 0.5rem', borderRadius: 'var(--radius-full)', background: pool.role === 'admin' ? 'var(--teal-faint)' : 'var(--surface-2)', color: pool.role === 'admin' ? 'var(--teal)' : 'var(--text-3)', fontFamily: 'var(--font-display)', fontWeight: 600 }}>{pool.role}</span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                        <span style={{ fontSize: '0.72rem', color: 'var(--text-3)', background: 'var(--surface-2)', padding: '0.15rem 0.5rem', borderRadius: 'var(--radius-full)' }}>{pool.pool_type === 'fiat' ? 'NGN pool' : 'Crypto pool'}</span>
                        {pool.pool_type === 'fiat' && <ArrowRight size={14} color="var(--teal)" />}
                      </div>
                    </div>
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-3)', marginBottom: '0.75rem' }}>
                      {pool.member_count} member{pool.member_count !== 1 ? 's' : ''} · {pool.pool_type === 'crypto' ? '0.25% fee' : '0.3% fee · Send to any bank'}
                    </div>
